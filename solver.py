...
def decompress(run, sys, melt, gas, system):
    ...
    def coh_eqs(guesses=[], fe=True, graph=False):
        ...
        for g in guesses:
            if np.isnan(g):
                ...
                guesses[np.where(guesses == g)] = 1e20
    ...
    def cohsn_new_eqs(guesses=[], fe=True, graph=False):
        """
        ...
        for g in guesses:
            if np.isnan(g):
                ...
                guesses[np.where(guesses == g)] = 1e-5
        
        w, x, y, z , mCO2 , mSO2, mH2, mH2O, mH2S, mCH4 = guesses[0], guesses[1], guesses[2], guesses[3], guesses[4],guesses[5], guesses[6], guesses[7], guesses[8], guesses[9]
    ...
    def cohsn_eqs(guesses=[], fe=True, graph=False,gassys =0.0020551190679259537):
        """
        Defines the non-linear equation set to be solved for the COHSN system.
                    ...
                    if run.GAS_SYS == "OH":
                        ...
                        if melt.graphite_sat is True:
                            newguess = optimize.fsolve(
                                ...
                                args=(run.FE_SYSTEM, melt.graphite_sat,gas),
                            )
    ...
    # COH system -----------------------------------------------------------------------
        ...
        try:
            guessw, guessx, guessy, guessz,guessl,guessm,guessn,guesso,guessp,guessq = jac_newton(run, gas, system)
        except RuntimeError:
            del sys.P_track[-1]  # delete failed pressure step
            msgs.earlyexit(
                sys,
        ...
        '''
        gas.mCO2.append(guessl)
        gas.mSO2.append(guessm)
        gas.mH2.append(guessn)
        gas.mH2O.append(guesso)
        gas.mH2S.append(guessp)
        gas.mCH4.append(guessq)
            


        #print(guessl,guessm,guessn,guesso,guessp,guessq)
        #print(gas.mCO2[-1] , gas.mSO2[-1], gas.mH2[-1], gas.mH2O[-1], gas.mH2S[-1], gas.mCH4[-1] )
        #print(guessx+guessy+guessw+guessz+guessl+guessm+guessn+guesso+guessp+guessq)
        gas.fo2.append(np.log(O2.Y * gas.mO2[-1] * sys.P))
        melt.fmq.append(cnvs.fo2_2fmq(gas.fo2[-1], sys.T, sys.P, sys.run.FMQ_MODEL))
        gas.get_fugacity(
            [H2O, H2, CO2, CO, CH4, S2, SO2, H2S, N2],
